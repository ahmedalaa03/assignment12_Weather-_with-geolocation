...
function getLocationFromGeoAPI(days) {
    if ("geolocation" in navigator) {
        navigator.geolocation.getCurrentPosition(
            function (position) {
                const latitude = position.coords.latitude;
                const longitude = position.coords.longitude;
                getCityName(latitude, longitude, days);
            }
        );
    }
}
function getCityName(lat, lon, days) {
    ...
    fetch(apiUrl)
        ...
        .then(data => {
            if (data && data.address && (data.address.city || data.address.town)) {
                ...
                fetchWeatherData(cityName, days);
            }
        })
}
...
document.getElementById('numberOfDays').addEventListener('change', function () {
    if (document.getElementById('location').value === '') {
        getLocationFromGeoAPI(document.getElementById('numberOfDays').value);
    } else {
